...
public sealed class MariaDbBuilder : ContainerBuilder<MariaDbBuilder, MariaDbContainer, MariaDbConfiguration>
{
    ...
    private sealed class WaitUntil : IWaitUntil
    {
        ...
        {
            _command = new List<string> { "mysql", $"--port={MariaDbPort}", $"--user={configuration.Username}", $"--password={configuration.Password}", configuration.Database, "--wait", "--silent", "--execute=SELECT 1;" };
        }
