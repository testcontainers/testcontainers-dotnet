namespace DotNet.Testcontainers.Clients
{
  ...
  {
    public DockerContainerOperations(
      Guid sessionId,
    ...
    public async Task<IEnumerable<ContainerListResponse>> GetAllAsync(
      CancellationToken ct = default
    )
    {
      return await DockerClient
        .Containers.ListContainersAsync(new ContainersListParameters { All = true }, ct)
    ...
    {
      return await DockerClient.Containers.InspectContainerAsync(id, ct).ConfigureAwait(false);
    }
    ...
    {
      var response = await DockerClient.Containers.WaitContainerAsync(id, ct).ConfigureAwait(false);
    ...
    )
    {
      var logsParameters = new ContainerLogsParameters
      {
        ShowStdout = true,
        ShowStderr = true,
        Since = Math.Max(0, Math.Floor(since.TotalSeconds))
          .ToString("0", CultureInfo.InvariantCulture),
        Until = Math.Max(0, Math.Floor(until.TotalSeconds))
          .ToString("0", CultureInfo.InvariantCulture),
        Timestamps = timestampsEnabled,
      };

      using (
        var stdOutAndErrStream = await DockerClient
          .Containers.GetContainerLogsAsync(id, logsParameters, ct)
          .ConfigureAwait(false)
      )
      {
        return await stdOutAndErrStream.ReadOutputToEndAsync(ct).ConfigureAwait(false);
      }
    }
    ...
    public Task RemoveAsync(string id, CancellationToken ct = default)
    {
      ...
      return DockerClient.Containers.RemoveContainerAsync(
        id,
    ...
    )
    {
      ...
      return DockerClient.Containers.ExtractArchiveToContainerAsync(
        id,
        new ContainerPathStatParameters { Path = path },
        tarStream,
        ct
      );
    }
    ...
    )
    {
      ...
      var tarResponse = await DockerClient
        .Containers.GetArchiveFromContainerAsync(
          id,
          new ContainerPathStatParameters { Path = path },
          false,
          ct
    ...
    )
    {
      ...
      var stream = await DockerClient
        .Containers.AttachContainerAsync(id, attachParameters, ct)
        .ConfigureAwait(false);

      _ = stream
        .CopyOutputToAsync(Stream.Null, outputConsumer.Stdout, outputConsumer.Stderr, ct)
        .ConfigureAwait(false);
    }
    ...
    )
    {
      ...
      var execCreateResponse = await DockerClient
        .Exec.CreateContainerExecAsync(id, execCreateParameters, ct)
        .ConfigureAwait(false);

      using (
        var stdOutAndErrStream = await DockerClient
          .Exec.StartContainerExecAsync(
      ...
      {
        var (stdout, stderr) = await stdOutAndErrStream
          .ReadOutputToEndAsync(ct)
          .ConfigureAwait(false);

        var execInspectResponse = await DockerClient
          .Exec.InspectContainerExecAsync(execCreateResponse.ID, ct)
          .ConfigureAwait(false);
    ...
    )
    {
      ...
      var networkingConfig = new NetworkingConfig { EndpointsConfig = converter.Networks };
      ...
      var createContainerResponse = await DockerClient
        .Containers.CreateContainerAsync(createParameters, ct)
        .ConfigureAwait(false);
