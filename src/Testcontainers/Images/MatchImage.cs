namespace DotNet.Testcontainers.Images
{
  ...
  internal static class MatchImage
  {
    ...
    {
      _ = Guard.Argument(image, nameof(image)).NotNull().NotEmpty();

      const string invalidReferenceFormat =
        "Error parsing reference: '{0}' is not a valid repository/tag.";

      var referenceMatch = ReferenceRegex.Instance.Match(image);

      _ = Guard
        .Argument(referenceMatch, nameof(image))
        .ThrowIf(
          argument => !argument.Value.Success,
          argument => new ArgumentException(
            string.Format(CultureInfo.InvariantCulture, invalidReferenceFormat, image),
            argument.Name
          )
        );
      ...
      var (registry, repository) =
        slices.Length == 2 && slices[0].LastIndexOfAny(['.', ':']) > -1
          ? (slices[0], slices[1])
          : (null, remoteName);
    ...
    private sealed class ReferenceRegex : Regex
    {
      private const string Pattern =
        "^(?<remote_name>(?:(?:(?:[a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9])(?:\\.(?:[a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]))*|\\[(?:[a-fA-F0-9:]+)\\])(?::[0-9]+)?\\/)?[a-z0-9]+(?:(?:[._]|__|[-]+)[a-z0-9]+)*(?:\\/[a-z0-9]+(?:(?:[._]|__|[-]+)[a-z0-9]+)*)*)(?::(?<tag>[\\w][\\w.-]{0,127}))?(?:@(?<digest>[A-Za-z][A-Za-z0-9]*(?:[-_+.][A-Za-z][A-Za-z0-9]*)*[:][0-9A-Fa-f]{32,}))?$";
      ...
      public static Regex Instance { get; } = new ReferenceRegex();
    }
  }
}
