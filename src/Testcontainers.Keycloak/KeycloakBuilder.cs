...
public sealed class KeycloakBuilder : ContainerBuilder<KeycloakBuilder, KeycloakContainer, KeycloakConfiguration>
{
    ...
    public override KeycloakContainer Build()
    {
        ...
        var isLatestVersion = "latest".Equals(DockerResourceConfiguration.Image.Tag, StringComparison.OrdinalIgnoreCase)
            || "nightly".Equals(DockerResourceConfiguration.Image.Tag, StringComparison.OrdinalIgnoreCase);
        ...
        var keycloakBuilder = DockerResourceConfiguration.WaitStrategies.Any() ? this : WithWaitStrategy(waitStrategy);
