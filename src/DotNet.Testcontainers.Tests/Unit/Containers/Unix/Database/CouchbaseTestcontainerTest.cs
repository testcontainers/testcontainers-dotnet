  ...
  public class CouchbaseTestcontainerTest
  {
    ...
    public async Task ConnectionEstablished()
    {
      ...
      var couchbaseTestcontainer = new TestcontainersBuilder<CouchbaseTestcontainer>()
        .WithDatabase(new CouchbaseTestcontainerConfiguration("Administrator", "password", "dotnet-testcontainers"))
      ...
      //When
      var cluster = await Cluster.ConnectAsync(couchbaseTestcontainer.Hostname, "Administrator","password");
      var bucket = await cluster.BucketAsync("dotnet-testcontainers");
      var pingReport = await bucket.PingAsync();

      //Then
      Assert.NotNull(pingReport);
    ...
    [Fact]
    public async Task CrudTest_WithRecentlyCreatedBucket()
    {
      //Given
      var couchbaseTestcontainer = new TestcontainersBuilder<CouchbaseTestcontainer>()
        .WithDatabase(new CouchbaseTestcontainerConfiguration("Administrator", "password", "dotnet-testcontainers"))
      ...
      //When & Then
      await couchbaseTestcontainer.CreateBucket("Customer",10);
    ...
    public async Task FlushBucket()
    {
      ...
      await collection.InsertAsync(
        "customer-id-1",
        new Customer {Name = "Andre", Age = 29}
      );

      await couchbaseTestcontainer.FlushBucket("Customer",10);
