  ...
  public class CouchbaseTestcontainerTest
  {
    ...
    public async Task ConnectionEstablished_WithCrudExample()
    {
      ...
      var couchbaseTestcontainer = new TestcontainersBuilder<CouchbaseTestcontainer>()
        ...
        .WithOutputConsumer(Consume.RedirectStdoutAndStderrToStream(OutStream, ErrorStream))
        .WithWaitStrategy(Wait.ForUnixContainer().UntilMessageIsLogged(OutStream, "couchbase-dev started"))
        .Build();
