namespace DotNet.Testcontainers.Tests.Unit.Containers.Unix.Database
{
  ...
  public class CouchbaseTestcontainerTest
  {
    ...
    {
      await this.StopAndRemoveOtherCouchbaseContainers();
      //Given
      var couchbaseTestcontainer = new TestcontainersBuilder<CouchbaseTestcontainer>()
        .WithDatabase(new CouchbaseTestcontainerConfiguration("Administrator", "password", "Customer"))
    ...
    private async Task StopAndRemoveOtherCouchbaseContainers()
    {
      var docker = new DockerContainerOperations(DockerApiEndpoint.Local);
      var containers = await docker.GetAllAsync();
      foreach (var container in containers.Where(p=>p.Image == CouchbaseTestcontainerConfiguration.CouchbaseImage))
      {
        await docker.StopAsync(container.ID);
        await docker.RemoveAsync(container.ID);
      }
    }
