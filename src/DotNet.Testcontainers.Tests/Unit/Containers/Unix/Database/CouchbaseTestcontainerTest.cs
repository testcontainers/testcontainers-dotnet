namespace DotNet.Testcontainers.Tests.Unit.Containers.Unix.Database
{
  ...
  public class CouchbaseTestcontainerTest
  {
    ...
    public async Task ConnectionEstablished_WithCrudExample()
    {
      ...
      Thread.Sleep(30000);
      this.testOutputHelper.WriteLine("Couchbase OutStream; ");
      this.testOutputHelper.WriteLine(Encoding.ASCII.GetString(OutStream.ToArray()));

      this.testOutputHelper.WriteLine("Couchbase ErrorStream; ");
      this.testOutputHelper.WriteLine(Encoding.ASCII.GetString(ErrorStream.ToArray()));
      //When & Then
