...
public sealed partial class DeclineLicenseAgreementTest
{
    ...
    {
        var exception = Assert.Throws<ArgumentException>(() => new EventHubsBuilder(TestSession.GetImageFromDockerfile()).Build());
    ...
    {
        var exception = Assert.Throws<ArgumentException>(() => new EventHubsBuilder(TestSession.GetImageFromDockerfile()).WithAcceptLicenseAgreement(false).Build());
        Assert.Matches(LicenseAgreementNotAccepted(), exception.Message);
    }
}