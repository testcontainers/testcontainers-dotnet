...
public sealed class FirestoreContainerTest : IAsyncLifetime
{
    ...
    {
        await _firestoreContainer.StartAsync().ConfigureAwait(false);
    }
    ...
    public async Task GetSnapshotReturnsSetDocument()
    {
        ...
        _ = await firestoreDb
            ...
            .Document()
            .SetAsync(documentData, cancellationToken: TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        ...
        Assert.Equal(
            documentData,
            querySnapshot
                .Documents.Select(document => document.ConvertTo<Dictionary<string, string>>())
                .Single()
        );
    }
}
