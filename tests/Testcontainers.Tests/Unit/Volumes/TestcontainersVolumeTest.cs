namespace DotNet.Testcontainers.Tests.Unit
{
  ...
  public sealed class TestcontainersVolumeTest : IClassFixture<VolumeFixture>, IAsyncLifetime
  {
    ...
    public TestcontainersVolumeTest(VolumeFixture volumeFixture)
    {
      ...
      _testcontainer1 = testcontainersBuilder.WithHostname(nameof(_testcontainer1)).Build();

      _testcontainer2 = testcontainersBuilder.WithHostname(nameof(_testcontainer2)).Build();
    }
    ...
    {
      await Task.WhenAll(
          _testcontainer1.DisposeAsync().AsTask(),
          _testcontainer2.DisposeAsync().AsTask()
        )
        .ConfigureAwait(false);
    }
    ...
    public async Task WithVolumeMount()
    {
      Assert.Equal(
        0,
        (
          await _testcontainer1.ExecAsync(
            new[] { "test", "-f", Path.Combine(Destination, nameof(_testcontainer2)) },
      ...
      Assert.Equal(
        0,
        (
          await _testcontainer2.ExecAsync(
            new[] { "test", "-f", Path.Combine(Destination, nameof(_testcontainer1)) },
            TestContext.Current.CancellationToken
          )
        ).ExitCode
      );
