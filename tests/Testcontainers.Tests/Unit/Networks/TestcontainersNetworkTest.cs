namespace DotNet.Testcontainers.Tests.Unit
{
  ...
  public sealed class TestcontainersNetworkTest : IClassFixture<NetworkFixture>, IAsyncLifetime
  {
    ...
    public TestcontainersNetworkTest(NetworkFixture networkFixture)
    {
      ...
      _container1 = containerBuilder.WithNetworkAliases(nameof(_container1) + AliasSuffix).Build();

      _container2 = containerBuilder.WithNetworkAliases(nameof(_container2) + AliasSuffix).Build();
    }

    public async ValueTask InitializeAsync()
    {
      await Task.WhenAll(_container1.StartAsync(), _container2.StartAsync()).ConfigureAwait(false);
    }
    ...
    public async Task PingContainer()
    {
      ...
      // When
      var execResult = await _container1
        .ExecAsync(new[] { "ping", "-c", "1", destination }, TestContext.Current.CancellationToken)
        .ConfigureAwait(true);
